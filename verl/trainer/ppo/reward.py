...
def get_step_custom_reward_fn(config):
    import importlib.util
    import sys
    ...
    if not file_path:
        return None
    file_path = file_path.replace(".py", "_with_steps.py")
...
def load_step_reward_manager(config, tokenizer, num_examine, **reward_kwargs):
    """
    Load and initialize a reward manager based on the configuration.
    ...
    # Try to get a custom reward function based on the configuration
    compute_score = get_step_custom_reward_fn(config)
    final_compute_score = compute_score
