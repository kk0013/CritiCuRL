...
from verl.trainer.ppo.ray_trainer_all import RayPPOTrainer
from verl.trainer.ppo.reward import load_reward_manager, load_step_reward_manager
...
class TaskRunner:
    def run(self, config):
        ...
        # # validate阶段的奖励函数也修改为带有步骤奖励的
        # val_reward_fn = load_step_reward_manager(
        #     config, tokenizer, num_examine=1, **config.reward_model.get("reward_kwargs", {})
        # )
        step_reward_fn = load_step_reward_manager(
            config, tokenizer, num_examine=0, **config.reward_model.get("reward_kwargs", {})
        )
